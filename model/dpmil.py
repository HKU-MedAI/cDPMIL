...
class DirichletProcess(nn.Module):
    def __init__(self, concentration, trunc, eta, batch_size, dim=1024, n_sample=100):
        ...
        self.mu = nn.ParameterList([nn.Parameter(torch.FloatTensor(self.dim).uniform_(-0.5, 0.5)) for t in range(self.T)])
        # self.sig = nn.Parameter(torch.stack([torch.eye(self.dim) for _ in range(self.T)]))
        self.rho = nn.ParameterList([nn.Parameter(torch.FloatTensor(self.dim).uniform_(-0.5, 0.5)) for t in range(self.T)])
...
#         logits = torch.log(pi).to(device)+log_pdfs
