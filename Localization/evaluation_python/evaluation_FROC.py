...
if __name__ == "__main__":
    ...
    for item in label_list:
        labels[item.split(',')[0].split('/')[-1].split('.')[0]]=int(item.split(',')[1])
    EVALUATION_MASK_LEVEL = 5 # Image level at which the evaluation is done
    L0_RESOLUTION = 0.243 # pixel resolution at level 0
    
    FROC_data = np.zeros((4, len(mask_folder)), dtype=object)
    FP_summary = np.zeros((2, len(mask_folder)), dtype=object)
    detection_summary = np.zeros((2, len(mask_folder)), dtype=object)
    
    caseNum = 0
    for case in mask_folder:
