...
def Get_Aligned_Label(dataset):
    clinical_data = pd.read_csv('/data1/WSI/Patches/'+dataset+'/BioData/clinical.tsv',sep='\t')
    ...
    for i in range(len(stage)):
        if '--' in stage.iloc[i,1]:
            ...
            stage.iloc[i,1]=1
    graph_list = glob.glob('/data1/WSI/Patches/' + dataset + '_Kimia_20x/*')
    ...
    aligned_label.to_csv('/data1/WSI/Patches/'+dataset+'/BioData/'+dataset+'_Stage_Label.csv',index=False)
