...
def process_COAD():
    ...
    graph_path = os.path.join(dataset_pth, 'COAD_kimia_lv0/homogeneous')
...
def split_camelyon16():
    ...
    test_list_txt = open('datasets/Camelyon/remix_processed/test_list.txt', 'w')
    train_labels, test_labels = [], []
    for i in tqdm(range(len(bags_path)), desc='splitting Camelyon'):
    ...
    train_labels = np.array(train_labels)
    test_labels = np.array(test_labels)
    np.save('datasets/Camelyon/remix_processed/test_bag_labels.npy', test_labels)
    np.save('datasets/Camelyon/remix_processed/train_bag_labels.npy', train_labels)
    print('done')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='Camelyon', choices=['Camelyon', 'Unitopatho', 'COAD'])
